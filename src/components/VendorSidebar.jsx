...
function SidebarContent({ setIsOpen, onLogout }) {
  ...
  const routes = [
    { name: "Vendor Profile", path: "/vendor/dashboard/profile", icon: FiUser },
    {
      name: "Add Ticket",
      ...
      <div className="p-6 border-b border-slate-900 flex items-center justify-between">
        <Link href="/" className="flex items-center space-x-2.5 group">
          ...
          <span className="text-md font-bold tracking-tight text-white">
            Routemate
            <span className="text-amber-500 font-medium text-xs ml-1.5 px-1.5 py-0.5 bg-amber-950/60 rounded-md border border-amber-900/60">
              Vendor
            </span>
          </span>
        </Link>
...
export default function VendorSidebar({ isOpen, setIsOpen, onLogout }) {
  return (
    <>
